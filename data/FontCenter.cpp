...
namespace FontSetting {
	...
	const char SuperMarioBros_font_path[] = "./assets/font/SuperMarioBros.ttf";
...
FontCenter::init() {
	for(const int &fs : FontSize::list) {
		...
		SuperMarioBros[fs] = al_load_ttf_font(FontSetting::SuperMarioBros_font_path, fs, 0);
...
FontCenter::~FontCenter() {
	...
	for(auto &[size, font] : courier_new)
		al_destroy_font(font);
	for(auto &[size, font] : SuperMarioBros)
		al_destroy_font(font);
